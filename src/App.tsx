...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedMatches) setResults(savedMatches);
  }, []);
  ...
  }, [jobs, candidates, activeBridge]);

  const handleAddJob = (newJob: Job) => {
    dataService.addJob(newJob);
    const updatedJobs = dataService.getJobs();
    setJobs(updatedJobs);
  };

  const handleAddCandidate = (newCandidate: Candidate) => {
    dataService.addCandidate(newCandidate);
    const updatedCandidates = dataService.getCandidates();
  ...
  const medalists = useMemo(() =>
    candidates.filter(c => c.silver_medalist.is_true),
  [candidates]);
      ...
      <aside className="w-64 bg-[#0f172a] text-white flex flex-col sticky top-0 h-screen shadow-2xl z-50 flex-shrink-0">
        ...
        <div className="px-4 pt-6 space-y-2">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3.5 mb-2 rounded-2xl bg-orange-600 text-white shadow-xl shadow-orange-900/40 text-xs font-black uppercase tracking-widest hover:scale-[1.02] transition-all"
          >
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3.5 mb-4 rounded-2xl bg-slate-700 text-white text-xs font-black uppercase tracking-widest hover:bg-slate-600 transition-all"
          >
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
          ...
          {navItems.map(item => (
            ...
            >
              <div className={`w-1.5 h-1.5 rounded-full ${activeTab === item.id ? 'bg-orange-500' : 'bg-slate-700'}`}></div>
              {item.label}
        ...
        <div className="mt-auto p-6 border-t border-slate-800">
          <div className="bg-slate-800/50 p-4 rounded-2xl">
            <div className="text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Active Bridge</div>
                  ...
                  <div className="flex-shrink-0 w-80">
                    ...
                    <div className="space-y-4">
                      {medalists.length === 0 ? (
                        ...
                        medalists.map(c => (
                          <EntityCard key={c.candidate_id} data={c} type="candidate" />
                        ))
                      )}
                    </div>
                  </div>

                  {/* Col 2: Match Pipeline */}
                  ...
                  <div className="flex-shrink-0 w-80">
                    ...
                    <div className="space-y-4">
                      {jobs.slice(0, 5).map(j => (
                        <EntityCard key={j.job_id} data={j} type="job" />
                      ))}
                      {jobs.length === 0 && (
                        <div className="h-32 border-2 border-dashed border-slate-300 rounded-3xl flex items-center justify-center text-center p-6">
                ...
                <div>
                  ...
                  {filteredJobs.length === 0 ? (
                    <div className="text-center py-32 bg-white rounded-[3rem] border border-slate-200">
                      ...
                      <p className="text-xs text-slate-300 mt-2">Use "Ingest JD" to add your first role</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {filteredJobs.map(j => (
                        <EntityCard
                          ...
                          data={j}
                          type="job"
                          onDelete={() => handleDeleteJob(j.job_id)}
                        />
                      ))}
                    </div>
                  )}
                ...
                <div>
                  ...
                  {filteredCandidates.length === 0 ? (
                    <div className="text-center py-32 bg-white rounded-[3rem] border border-slate-200">
                      ...
                      <p className="text-xs text-slate-300 mt-2">Add your first silver medalist</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {filteredCandidates.map(c => (
                        <EntityCard
                          ...
                          data={c}
                          type="candidate"
                          onDelete={() => handleDeleteCandidate(c.candidate_id)}
                        />
                      ))}
                    </div>
                  )}
